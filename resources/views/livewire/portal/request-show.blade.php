<div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-gray-200 dark:border-gray-700 pb-6 mb-8">
        ...
        <div class="flex items-center gap-3">
            <flux:button href="{{ route('portal.requests') }}" variant="ghost" icon="arrow-left">
                Volver
            </flux:button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            {{-- Documents --}}
            @php
                $documents = is_array($clientRequest->documents) ? $clientRequest->documents : (is_string($clientRequest->documents) ? json_decode($clientRequest->documents, true) : []);
            ...
            @if(count($documents) > 0)
                <div class="bg-white dark:bg-sidebar-dark rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
                    <h3 class="flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white mb-4">
                        ...
                        Documentos Adjuntos ({{ count($documents) }})
                    </h3>
        ...
        <div class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                {{-- Request Types --}}
                @php
                    $requestTypes = is_array($clientRequest->request_types) ? $clientRequest->request_types : (is_string($clientRequest->request_types) ? json_decode($clientRequest->request_types, true) : []);
                    if(!is_array($requestTypes)) $requestTypes = [$clientRequest->request_types];
                ...
                {{-- Expected Results --}}
                @php
                    $expectedResults = is_array($clientRequest->expected_results) ? $clientRequest->expected_results : (is_string($clientRequest->expected_results) ? json_decode($clientRequest->expected_results, true) : []);
                    if(!is_array($expectedResults)) $expectedResults = [$clientRequest->expected_results];
                    $expectedResults = array_filter($expectedResults);
                @endphp
